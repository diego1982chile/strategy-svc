...
import cl.dsoto.trading.config.AppConfig;
...
public class AsyncService {
    ...
    @Transactional
    public void updateStatus(WFO wfo, Status status) {
        wfo.setStatus(status);
        WFOEntity wfoEntity = wfoMapper.toEntity(wfo);
        wfoRepository.save(wfoEntity);
    }

    @Asynchronous
    public Future process(WFO wfo) throws Exception {

        WFOEntity wfoEntity = wfoRepository.findById(wfo.getId());
        WFO persistedWFO = wfoMapper.toDomain(wfoEntity);

        final int lastPersistedK = persistedWFO.getWfoRecords().stream()
                .map(WFORecord::getIteration)
                .max(Integer::compare).orElse(0);

        int lastPersistedI = (int) persistedWFO.getWfoRecords().stream()
                .filter(e -> e.getIteration() == lastPersistedK)
                .count();
        ...
        for (int k = lastPersistedK; k < wfo.getIterations(); ++k) {
            ...
            for (Strategy strategy : strategies) {
                ...
                for (int i = lastPersistedI; i <= iterations; ++i) {

                    org.springframework.data.util.Pair<Integer,Integer> trainInterval = barService.getTrainRange(wfo, data.getBarData(), wfo.getStart().getYear() + i);
                    org.springframework.data.util.Pair<Integer,Integer> testInterval = barService.getTestRange(wfo, data.getBarData(), wfo.getStart().getYear() + i);

                    String nameIn = String.valueOf(data.getBar(trainInterval.getFirst()).getBeginTime().getYear());
                    nameIn = nameIn + "_" + String.valueOf(data.getBar(trainInterval.getSecond()).getBeginTime().getYear());

                    String nameOut = String.valueOf(data.getBar(testInterval.getFirst()).getBeginTime().getYear());
                    nameOut = nameOut + "_" + String.valueOf(data.getBar(testInterval.getSecond()).getBeginTime().getYear());

                    TimeSeries in = new BaseTimeSeries(nameIn,data.getBarData().subList(trainInterval.getFirst(), trainInterval.getSecond()));
                    TimeSeries out = new BaseTimeSeries(nameOut,data.getBarData().subList(testInterval.getFirst(),testInterval.getSecond()));
        ...
        updateStatus(wfo, Status.COMPLETED);
    ...
    private BackTest createFromSeries(TimeSeries timeSeries) throws Exception {
        ...
        switch ((int) Duration.between(timeSeries.getBar(0).getBeginTime(), timeSeries.getBar(1).getBeginTime()).getSeconds()) {
            ...
            case 259200:
                ...
                break;
            default:
                log.error(("unsupported TimeFrame: " + (int) Duration.between(timeSeries.getBar(0).getBeginTime(), timeSeries.getBar(1).getBeginTime()).getSeconds()));
    ...
    private WFORecord computeResults(int iteration, BackTest selected, Strategy strategy,
                                       String currentPeriod, String previousPeriod, String stage, int correlative) {

        try {

            System.out.println("Computing results for strategy: " + strategy.getName());
            ...
            */
            ...
            log.error("Unexpected exception computing results: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private static List<org.ta4j.core.Strategy> mapFrom(BackTest backTest) throws Exception {
    ...
    private static Map<String, List<Pair<String, Integer>>> mapStrategiesFrom(BackTest period) {
